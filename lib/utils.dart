...
int getBit(Uint8List data, int bitNum) =>
    ((data[bitNum ~/ 8] & (1 << (7 - bitNum % 8))) == 0) ? 0 : 1;
...
String parsePacket(FreestyleLibrePacket packet) {
  ...
  out.write(
      "Sensor age: ${packet.sensorAge} min, first use: ${packet.sensorFirstUse}, read time: ${packet.readDate}\n");
...
String parseBTPacket(TomatoBridgePacket packet) {
  ...
  out.write(
      "Sensor patch: uid=${toHex(packet.patchUid)}, info=${toHex(packet.patchInfo)}\n");
