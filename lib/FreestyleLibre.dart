...
bool libreCRC(Uint8List data) =>
    Crc16Mcrf4xx().convert(data.sublist(2)) ==
    reverseBits(data[1] << 8 | data[0], 16);
...
class FreestyleLibreGlucoseData
    with CalibrableGlucoseDataMixin
    implements GlucoseData {
  FreestyleLibrePacket packet;
  int index;
  bool historical;
  num calibrationFactor;

  FreestyleLibreGlucoseData(this.packet, this.index, this.calibrationFactor,
      {this.historical = false});

  int get _i {
    // ring buffer index
    var i =
        (historical ? packet._indexHistory : packet._indexTrend) - index - 1;
  ...
  int get rawValue {
    ...
    return (packet._data[_i * 6 + offset] |
            packet._data[_i * 6 + offset + 1] << 8) &
        0x1FFF;
  }

  int get sensorTime => max(
      0,
      historical
  ...
  @override
  int get hashCode =>
      packet?.serialNumber.hashCode ^ sensorTime.hashCode ^ rawValue.hashCode;
...
class FreestyleLibrePacket {
  ...
  FreestyleLibrePacket(this._data, this._calibrationFactor,
      {this.serialNumber, DateTime readDate, this.source}) {
    readDate ??= DateTime.now();
    this.readDate = DateTime(readDate.year, readDate.month, readDate.day,
        readDate.hour, readDate.minute);
  }

  FreeStyleLibreSensorStatus get status =>
      _data[4] < FreeStyleLibreSensorStatus.values.length
          ? FreeStyleLibreSensorStatus.values[_data[4]]
          : FreeStyleLibreSensorStatus.unknown;

  bool get sensorReady => [
        FreeStyleLibreSensorStatus.starting,
        FreeStyleLibreSensorStatus.ready
      ].contains(status);
