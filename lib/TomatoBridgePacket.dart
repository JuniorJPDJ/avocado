...
const TOMATO_MIN_PACKET_LENGTH = TOMATO_HEADER_LENGTH + FREESTYLELIBRE_PACKET_LENGTH + 1;
...
class TomatoBridgePacket {
  ...
  Uint8List get patchInfo =>
      _data.length >= TOMATO_MIN_PACKET_LENGTH + TOMATO_PATCH_SUFFIX_LENGTH
          ? _data.sublist(
          TOMATO_MIN_PACKET_LENGTH,
          TOMATO_MIN_PACKET_LENGTH + TOMATO_PATCH_SUFFIX_LENGTH
      ) : null;

  String get freestyleLibreSerialNumber {
    var snData = Uint8List.fromList(_data.sublist(5, 11).reversed.toList() + [0, 0]);
    String sn = '0';

    for(var i=0; i<10; ++i){
      sn += FREESTYLELIBRE_SN_LOOKUP[getBit(snData, i*5) << 4 | getBit(snData, i*5+1) << 3 | getBit(snData, i*5+2) << 2 | getBit(snData, i*5+3) << 1 | getBit(snData, i*5+4)];
    }
